...
def main():
    ...
    for rec in source_rows:
        ...
        # _Bin is quarantine/cleanup storage and is NEVER an implicit
        # restore source. Only files currently in MASTER are restorable.
        if str(storage_state).upper() == "IN_BIN":
            rows.append({
                "file_id": file_id,
                "sha256": sha,
                "filename": filename,
                ...
                "storage_state": storage_state or "",
                "catalog_status": status or "",
                "physical_status": "EXCLUDED_IN_BIN",
                "physical_size_bytes": "",
                "physical_sha256": "",
                "size_status": "NOT_CHECKED",
                "sha_status": "NOT_RUN",
                "restore_status": "EXCLUDED",
                "reason": "IN_BIN_NOT_IMPLICITLY_RESTORABLE",
            })
            continue
    ...
    excluded_bin = sum(r["restore_status"] == "EXCLUDED" for r in rows)
    blocked = sum(r["restore_status"] == "BLOCKED" for r in rows)
    ...
    print("READY            :", ready)
    print("EXCLUDED_IN_BIN  :", excluded_bin)
